...
import { CreateProductVariantStandaloneRequest, Product } from '../../../types/product';
...
export default function ProductVariantCreate({ 
  ...
  const loadProducts = async () => {
    try {
      ...
      // Set selected product if productId provided
      if (productId) {
  ...
  const handleSubmit = async () => {
    if (!formData.productId) {
      showDialog('error', 'Lỗi', 'Vui lòng chọn sản phẩm');
      return;
    }
    ...
    try {
      ...
      if (response?.data || response) {
        ...
        showDialog('success', 'Thành công', 'Tạo biến thể sản phẩm thành công');
      ...
      showDialog('error', 'Lỗi', err?.message || 'Không thể tạo biến thể sản phẩm');
    } finally {
      setSubmitting(false);
    }
  };
